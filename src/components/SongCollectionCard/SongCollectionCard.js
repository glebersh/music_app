...
const SongCollectionCard = (props) => {
  ...
  const { id } = props;
  const playlistType = props?.collectionType.toLowerCase();
  ...
  return (
    (<Flex m='1em 0 0 0' direction='column' align='center' justify='flex-start'
      ...
      {props?.collectionType === 'ALBUMS' && <Link to={`artists/${props?.artists[0]?.id}`}>
        <Text _hover={{ color: 'primary' }}>{props?.collectionType === 'ALBUMS' ? props?.artists[0]?.name : null}</Text></Link>}

      {props.collectionType === 'PLAYLISTS' && <Text>{props.collectionType === 'PLAYLISTS' && props?.owner?.display_name}</Text>}
